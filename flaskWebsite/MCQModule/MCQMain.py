...
def main():
    ...
    # MCQ page
        ...
        print(f"Attempted {attempted_count}/{len(attempted_questions)} questions.")
        ...
        if st.session_state.get("submitted", False):
            ...
            for i, (user_ans, key_ans, attempted) in enumerate(zip(user_answers, st.session_state["key_answers"], attempted_questions), start=1):
                if not attempted:
                    print(f"Question {i}: Not Attempted 🚫")
                else:
                    ...
                    if len(key_ans_parts) > 1:
                        ...
                        print(f"Question {i}: Answer not found")

            print(f"Total Score: {score}/{len(attempted_questions)} 🎉")
            ...
            for i, key_ans in enumerate(st.session_state["key_answers"], start=1):
                ...
                if len(key_ans_parts) > 1:
                    correct_ans = key_ans_parts[1].strip()
                    print(f"Q{i}. {correct_ans}")
                else:
                    print(f"Q{i}. Answer not found")
